    ...
    <div className="card border-purple-500/20 bg-gradient-to-br from-purple-900/10 to-cyan-900/10">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={reset}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-800 dark:bg-gray-800 bg-gray-200 text-gray-400 hover:text-gray-200 transition-colors"
          >
            <RotateCcw size={12} /> Reset
          </button>
          <button
            onClick={() => { setEnabled(false); reset(); }}
            className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-800 dark:bg-gray-800 bg-gray-200 text-gray-400 hover:text-gray-200 transition-colors"
          >
            Close
          </button>
        </div>
      ...
      {simResult && (
        <div className="border-t border-gray-800 dark:border-gray-800 border-gray-200 pt-4">
          <h4 className="text-sm font-medium text-gray-300 dark:text-gray-300 text-gray-700 mb-3">
          ...
          <div className="mt-3 space-y-1.5">
            {simResult.simplifiedGraph.map((edge, i) => (
              <div key={i} className="flex items-center justify-between text-xs p-2 rounded-lg bg-gray-800/50 dark:bg-gray-800/50 bg-gray-100">
                <span>
                  <span className="text-red-400">{edge.fromName}</span>
                  <span className="text-gray-600 dark:text-gray-600 text-gray-400"> → </span>
                  <span className="text-green-400 dark:text-green-400 text-green-600">{edge.toName}</span>
                </span>
                <span className="font-mono text-purple-300 dark:text-purple-300 text-purple-600">{formatCurrency(edge.amount)}</span>
              </div>
            ))}
          </div>
        </div>
